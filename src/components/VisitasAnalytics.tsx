...
export default function VisitasAnalytics({ data }: {
  data: { semanas: { label: string; count: number }[]; topInmuebles: { id: string; dir: string; count: number }[] };
}) {
  return (
    <div className="rounded-2xl border border-border bg-card p-5 flex flex-col">
      <h3 className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground font-medium mb-3">Visitas · últimas 8 semanas</h3>
      ...
      {data.topInmuebles.length > 0 && (
        <div className="mt-3 pt-3 border-t border-border flex-1">
          <div className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground mb-2">Top inmuebles</div>
          <div className="space-y-1.5">
            {data.topInmuebles.map((item, idx) => (
              ...
              >
                <span className="text-[10px] text-muted-foreground tabular-nums w-3 shrink-0">{idx + 1}</span>
