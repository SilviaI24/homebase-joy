...
import {
  Globe, MessageSquare, UserRound, Phone, Mail, ExternalLink,
  CheckCircle2, Loader2, Search, Hourglass, Plus, X,
} from "lucide-react";
  ...
  errorComponent: ({ error }) => (
    <AppShell title="Prospectos">
      <div className="rounded-md border border-destructive/40 bg-destructive/5 p-4 text-sm text-destructive">
        Error cargando prospectos: {error.message}
      </div>
    </AppShell>
  ),
});

const TIPOS_INMUEBLE = ["Piso", "Chalet", "Terreno", "Local", "Garaje", "Trastero", "Edificio",
  "Alquiler Piso", "Alquiler Chalet", "Alquiler Local", "Alquiler Oficina"];
...
const CANAL_META: Record<ProspectoCanal, CanalMeta> = {
  Web:     { label: "Web",     icon: Globe,         tabActive: "border-violet-500 text-violet-700 dark:text-violet-300", badgeActive: "bg-violet-500 text-white" },
  SilvIA:  { label: "SilvIA",  icon: MessageSquare, tabActive: "border-blue-500 text-blue-700 dark:text-blue-300",       badgeActive: "bg-blue-500 text-white"   },
  Directo: { label: "Directo", icon: UserRound,     tabActive: "border-amber-500 text-amber-700 dark:text-amber-300",    badgeActive: "bg-amber-500 text-white"  },
};
...
type NuevoForm = {
  ...
  tipo: string; calle: string; numero: string; localidad: string;
  precio: string; superficie: string; habitaciones: string;
};
const FORM_EMPTY: NuevoForm = {
  nombre: "", telefono: "", email: "",
  tipo: "Piso", calle: "", numero: "", localidad: "",
...
function ProspectosPage() {
  ...
  const set = (k: keyof NuevoForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
    ...
    <AppShell title="Prospectos" subtitle="Captación de nuevos propietarios">
          ...
          {CANALES.map((c) => {
            ...
            return (
              ...
              >
                ...
                {m.label}
                <span className={`min-w-[18px] h-[18px] flex items-center justify-center rounded-full text-[10px] font-bold leading-none px-1 transition-colors ${isActive ? m.badgeActive : "bg-muted text-muted-foreground"}`}>
                  {byCanal[c].length}
                </span>
              </button>
            );
      ...
      {current.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 gap-3 text-muted-foreground">
          ...
          <p className="text-sm">
            {q ? "Sin resultados para esa búsqueda." : `No hay prospectos ${meta.label.toLowerCase()} pendientes.`}
          </p>
          {!q && tab === "Directo" && (
            <button
              onClick={() => setShowNuevo(true)}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/50 border-b border-border">
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-muted-foreground">Propietario</th>
                  ...
                  <th className="text-left px-4 py-2.5 text-xs font-medium text-muted-foreground hidden lg:table-cell">Llegado</th>
                ...
                {current.map((p) => (
                  <tr key={p.id} className="bg-card hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-3">
                      ...
                      <div className="text-xs text-muted-foreground">{p.telefono || p.email || "—"}</div>
                    </td>
                    <td className="px-4 py-3 hidden sm:table-cell">
                      {p.inmueble ? (
                        <>
                          <div className="font-medium text-foreground">
                            {p.inmueble.calle}{p.inmueble.numero ? ` ${p.inmueble.numero}` : ""}{p.inmueble.localidad ? `, ${p.inmueble.localidad}` : ""}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            {[p.inmueble.tipo, p.inmueble.superficie && `${p.inmueble.superficie} m²`, p.inmueble.habitaciones && `${p.inmueble.habitaciones} hab.`, p.inmueble.precio && formatEuro(p.inmueble.precio)].filter(Boolean).join(" · ")}
                          </div>
                        </>
                      ) : (
                        <span className="text-xs text-muted-foreground italic">{p.motivo || "Sin inmueble vinculado aún"}</span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1.5">
                        {p.telefono && (
                          <a href={`tel:${p.telefono}`} title={`Llamar a ${p.nombre}`}
                        ...
                        {p.email && (
                          <a href={`mailto:${p.email}`} title={`Email a ${p.nombre}`}
                            className="inline-flex items-center justify-center size-8 rounded-md border border-border hover:bg-accent transition-colors text-muted-foreground hover:text-foreground">
                            <Mail className="size-3.5" />
                          </a>
                        )}
                        {p.inmueble && (
                          <Link to="/inmuebles/$id" params={{ id: p.inmueble.id }} title="Ver ficha del inmueble"
                            className="inline-flex items-center justify-center size-8 rounded-md border border-border hover:bg-accent transition-colors text-muted-foreground hover:text-foreground">
                        ...
                        >
                          {activating === p.id ? <Loader2 className="size-3.5 animate-spin" /> : <CheckCircle2 className="size-3.5" />}
                          <span className="hidden sm:inline">{activating === p.id ? "…" : "Activar"}</span>
                        </button>
                      </div>
                    </td>
                  </tr>
          ...
          <div className="w-full max-w-lg rounded-2xl border border-border bg-card shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Propietario contactado en persona o por teléfono</p>
              </div>
              <button onClick={() => { setShowNuevo(false); setForm(FORM_EMPTY); }}
            ...
            <form onSubmit={(e) => { e.preventDefault(); crearMut.mutate(); }}
              className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">

              {/* Propietario */}
              <div>
                <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground font-medium mb-2">Propietario</p>
                <div className="grid grid-cols-1 gap-2">
                  <input required value={form.nombre} onChange={set("nombre")} placeholder="Nombre completo *"
                    className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                  <div className="grid grid-cols-2 gap-2">
                    <input value={form.telefono} onChange={set("telefono")} placeholder="Teléfono"
              ...
              {/* Inmueble */}
              <div>
                <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground font-medium mb-2">Inmueble</p>
                <div className="grid grid-cols-1 gap-2">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <input value={form.localidad} onChange={set("localidad")} placeholder="Localidad"
                      className="h-9 rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <input required value={form.calle} onChange={set("calle")} placeholder="Calle *"
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <input value={form.precio} onChange={set("precio")} type="number" placeholder="Precio €"
                      className="h-9 rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                    <input value={form.superficie} onChange={set("superficie")} type="number" placeholder="m²"
                      className="h-9 rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                    <input value={form.habitaciones} onChange={set("habitaciones")} type="number" placeholder="Hab."
                      className="h-9 rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                  </div>
                </div>
              </div>
              ...
              {agData.agentes.length > 0 && (
                <div>
                  <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground font-medium mb-2">Agente responsable</p>
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button type="button" onClick={() => { setShowNuevo(false); setForm(FORM_EMPTY); }}
