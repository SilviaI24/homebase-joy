...
async function main() {
  ...
  for (const r of withImages) {
    ...
    const tasks = atImages.map(
      (att, i) => async (): Promise<{ url: string; filename: string; orden: number } | null> => {
    ...
    const newImagenes = results.filter(
      (r): r is { url: string; filename: string; orden: number } => r !== null,
    );
