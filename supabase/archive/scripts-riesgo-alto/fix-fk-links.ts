...
async function main() {
  ...
  console.log(
    `   agents: ${agentMap.size}, properties: ${propMap.size}, contacts: ${contactMap.size}\n`,
  );
  ...
  for (let i = 0; i < visitsWithLinks.length; i += CHUNK) {
    ...
    for (const r of chunk) {
      const f = r.fields;
      const property_id = arr(f["Inmuebles"])[0]
        ? (propMap.get(arr(f["Inmuebles"])[0]) ?? null)
        : null;
      const contact_id = arr(f["Clientes"])[0]
        ? (contactMap.get(arr(f["Clientes"])[0]) ?? null)
        : null;
  ...
  if (delErr) {
    console.error("Failed to delete contact_roles:", delErr.message);
    process.exit(1);
  }
