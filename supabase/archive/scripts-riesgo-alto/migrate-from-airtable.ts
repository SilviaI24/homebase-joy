...
const mapImages = (field: unknown) =>
  Array.isArray(field)
    ? (field as Att[])
        .map((a, i) => ({ url: attUrl(a), filename: a.filename ?? `img_${i}`, orden: i }))
        .filter((a) => a.url)
    : [];
...
const mapCicloVida = (tipo: string, pIds: string[], cIds: string[], aIds: string[]) => {
  ...
  if (pIds.length || cIds.length || aIds.length || t === "propietario" || t === "comprador")
    return "Activo";
...
async function main() {
  ...
  console.log(
    `   Fetched in ${((Date.now() - t0) / 1000).toFixed(1)}s: ${atAgentes.length} agents, ${atInmuebles.length} properties, ${atClientes.length} contacts, ${atVisitas.length} visits\n`,
  );
  ...
  const agentsDB = await fetchAllSupa<{ id: string; airtable_id: string }>(
    "agents",
    "id,airtable_id",
  );
  ...
  const propsDB = await fetchAllSupa<{ id: string; airtable_id: string }>(
    "properties",
    "id,airtable_id",
  );
  ...
  const contactsDB = await fetchAllSupa<{ id: string; airtable_id: string }>(
    "contacts",
    "id,airtable_id",
  );
  ...
  const resolvedRoleRows = roleRows
    .map((r) => ({
      contact_id: contactMap.get(r._at),
      tipo: r.tipo,
      estado: "Activo",
  ...
  for (let i = 0; i < resolvedAgentRows.length; i += CHUNK) {
    ...
    const { error } = await supa
      .from("contact_agents")
      .upsert(chunk, { onConflict: "contact_id,agent_id", ignoreDuplicates: true });
