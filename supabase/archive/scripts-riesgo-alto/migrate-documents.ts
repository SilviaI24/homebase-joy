...
async function main() {
  ...
  for (const r of withDocs) {
    ...
    const tasks = atDocs.map(
      (att, i) => async (): Promise<{ url: string; filename: string; type: string } | null> => {
        const url = att.url ?? "";
        if (!url) return null;
    ...
    const newDocs = results.filter(
      (r): r is { url: string; filename: string; type: string } => r !== null,
    );

    if (newDocs.length === 0) {
      console.log(`  ⚠  ${label}: no docs migrated`);
      continue;
    }
    ...
    const { error } = await supa
      ...
      .eq("id", supaProp.id);
    if (error) {
      console.warn(`  ✗ ${label}: update failed`);
      totalErrors++;
    } else {
      propsUpdated++;
      console.log(`  ✓ ${label}: ${newDocs.length}/${atDocs.length} docs`);
    }
